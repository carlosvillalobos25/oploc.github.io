...
(function (factory, window) {
    ...
    L.Control.BigImage = L.Control.extend({
        options: {
            ...
            macroName: "macro_localizacion.png",
            microName: "micro_localizacion.png"
        },

        onAdd: function (macromap, micromap) {
        ...
        _getLayers: function (resolve) {
            let self = this;
            let promises = [];
            self._macromap.eachLayer(function (layer) {
        ...
        _getMarkerLayer: function (layer, resolve) {
            ...
            let pixelPoint = self._macromap.project(layer._latlng);
        ...
        _getPathLayer: function (layer, resolve) {
            ...
            latlngs.forEach((latLng) => {
                let pixelPoint = self._macromap.project(latLng);
                pixelPoint = pixelPoint.subtract(new L.Point(self.bounds.min.x, self.bounds.min.y));
        ...
        _drawCircle: function (layer, resolve) {
            ...
            let point = this._macromap.project(layer._latlng);
        ...
        _print: function () {
            ...
            let dimensions = self._macromap.getSize();

            self.zoom = self._macromap.getZoom();
            self.bounds = self._macromap.getPixelBounds();
            ...
            promise.then(() => {
                ...
                self.canvas.toBlob(function (blob) {
                    ...
                    link.download = self.options.macroName;
